...
case class DeploymentConfig(
     ...
     *  Defaults to "ctdp<stage>dbxlakehouse" when absent, e.g. "ctdpdevdbxlakehouse".
     */
    ...
    /** Cluster settings. Entire block is optional — all fields have sensible defaults.
     *  Only override what differs from the defaults for your environment.
     */
    clusterConfiguration: DeploymentConfig.ClusterConfiguration = DeploymentConfig.ClusterConfiguration(),
    /** Optional path (relative to project root) to a resource YAML file or a
     *  directory of resource YAML files to include in the Databricks asset bundle.
     *  - A single file:  "resources/lakehouse_job.yml"
     *  - A directory:    "resources"  (all *.yml / *.yaml files in it are included)
     *  If absent, no extra resource files are included.
     */
    resourceFilesPath: Option[String] = None,
    /** Optional cron schedule for the job.
     *  If absent the job is unscheduled (triggered manually or via API).
     */
    schedule: Option[DeploymentConfig.ScheduleConfig] = None,
    /** "production" (default) or "development".
     *  In development mode the Databricks CLI ignores run_as and prefixes the
     *  job name with [dev <username>], so the job runs as the deploying identity
     *  (PAT owner or OAuth-M2M SP), not the jobRunIdentity.
     */
...
object DeploymentConfig {
   ...
   *
   *  "azure-client-secret" — Azure-registered service principal.
   *    Required fields: tenantId, clientId, clientSecret.
   *
   *  "oauth-m2m"           — Databricks-native service principal (OAuth M2M).
   *    Required fields: clientId, clientSecret.
   *
   *  "pat"                 — Personal access token (local dev / testing only).
  ...
  case class JobRunIdentity(
       ...
       *  1. The Unity Catalog schema name in the volume path (no special chars).
       *  2. The `service_principal_name` in `run_as` for production bundles.
       *  Use a service-principal UUID for production, or a short personal
       *  shortname (e.g. "uih00000") for development — run_as is omitted in
       *  development mode so the value is only used for the volume schema path.
       *  When absent, the volume schema defaults to "default" and run_as is omitted.
       */
      clientId: String
  )
  ...
  /** Quartz-based cron schedule for the Databricks job.
   *  quartzCronExpression — e.g. "0 0 * ? * *" for every hour on the hour.
   *  timezoneId           — IANA timezone, defaults to "UTC".
   *  pauseStatus          — "UNPAUSED" (run) or "PAUSED" (skip). Defaults to "UNPAUSED".
   */
