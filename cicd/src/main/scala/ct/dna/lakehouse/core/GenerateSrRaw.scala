...
object GenerateSrRaw extends LoggingTrait {
  ...
  private def persistAsts(baseDir: LocalDir, basePackage: String, catalogId: CatalogID): Unit = {
    ...
    tableAsts.foreach { case (tableId, ast) =>
  ...
  private def generateAsts(basePackage: String, catalogId: CatalogID): (
      Map[TableID, LoadedTableSpecAstBuilder.LoadedTableSpecWithEntityAst],
      Set[SchemaSpecAstBuilder.SchemaPackageObjectAst],
      CatalogSpecAstBuilder.CatalogPackageObjectAst
  ) = {

    logger.debug(s"Identifying table descriptions for catalog '${catalogId.name}'")
    val tableDescs = identifyTableDescs(catalogId)
  ...
  private def identifyTableDescs(catalogId: CatalogID): Map[TableID, TableDesc.UnityTableDesc] = {
