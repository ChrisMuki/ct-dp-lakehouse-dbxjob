# fin_regional_dashboard Workflow Docs

Per-table execution-flow documentation for the `ct.dna.lakehouse.dm_md.fin_regional_dashboard` package. Each doc describes how the table is populated from its sources at runtime, the merge/overwrite strategy used, and the per-table edge cases.
...
The tables form a DAG. The runner [`run-regional-dashboard-dm-tables.sh`](../../../../run-regional-dashboard-dm-tables.sh) executes them in this dependency order:
