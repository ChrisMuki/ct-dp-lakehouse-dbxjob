...
Denormalises material-master data: enriches every `(_mk_system, _mk_instance, matnr)` from `dm_mara` with its description from `dm_makt` and its material-group description from `dm_t023t`. One target row per material.
...
All three are dm-layer tables, so each is already 1-row-per-PK.
...
flowchart TD
    ...
    B -- no --> D[mara = changeFeeds dm_mara .toDF<br/>makt = broadcast dm_makt<br/>t023t = broadcast dm_t023t]
    D --> E[mara LEFT JOIN makt<br/>on _mk_system + _mk_instance + matnr]
    E --> F[LEFT JOIN t023t<br/>on _mk_system + _mk_instance + matkl]
    F --> G[select projected columns +<br/>key_column = system + instance + _ + matnr]
...
- Both `dm_makt` and `dm_t023t` are broadcast — they're small relative to `dm_mara`, so this becomes a single map-side stage.
- `spras` and `spras_t023t` are still selected, but they're the pivot's "preferred language" indicator, not row multipliers.

## Validation

`require(sourceTableSpecs.toSet == Set(dm_mara, dm_makt, dm_t023t))` — guards against an accidental change to the source list that could drop or add a join.
