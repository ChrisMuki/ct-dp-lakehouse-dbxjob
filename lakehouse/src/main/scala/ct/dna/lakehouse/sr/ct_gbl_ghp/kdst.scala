...
object kdst
    extends TableSpec[E_kdst](
      enableChangeDataFeed = true,
      clusterBy = ClusterBy.PRIMARY_KEY,
      timetravelDays = 35
    )
    with ChangeKey[sr_raw.ct_gbl_ghp.E_kdst] {
  override def sourceTableSpec: TableSpec[sr_raw.ct_gbl_ghp.E_kdst] = sr_raw.ct_gbl_ghp.kdst
...
sealed class C_kdst(prefix: String) extends ColumnWithNameAccessor {
  val mandt: ColumnWithName = ColumnWithName(prefix + "mandt")
  val vbeln: ColumnWithName = ColumnWithName(prefix + "vbeln")
  val vbpos: ColumnWithName = ColumnWithName(prefix + "vbpos")
...
object C_kdst extends C_kdst("") {
  def as(alias: String): C_kdst = new C_kdst(alias + ".")
}
