...
object jest
    extends TableSpec[E_jest](
      enableChangeDataFeed = true,
      clusterBy = ClusterBy.PRIMARY_KEY,
      timetravelDays = 35
    )
    with ChangeKey[sr_raw.ct_gbl_ghp.E_jest] {
  override def sourceTableSpec: TableSpec[sr_raw.ct_gbl_ghp.E_jest] = sr_raw.ct_gbl_ghp.jest
...
sealed class C_jest(prefix: String) extends ColumnWithNameAccessor {
  val mandt: ColumnWithName = ColumnWithName(prefix + "mandt")
  val objnr: ColumnWithName = ColumnWithName(prefix + "objnr")
  val stat: ColumnWithName = ColumnWithName(prefix + "stat")
...
object C_jest extends C_jest("") {
  def as(alias: String): C_jest = new C_jest(alias + ".")
}
