...
object trugt
    extends TableSpec[E_trugt](
      enableChangeDataFeed = true,
      clusterBy = ClusterBy.PRIMARY_KEY,
      timetravelDays = 35
    )
    with ChangeKey[sr_raw.ct_gbl_ghp.E_trugt] {
  override def sourceTableSpec: TableSpec[sr_raw.ct_gbl_ghp.E_trugt] = sr_raw.ct_gbl_ghp.trugt
...
sealed class C_trugt(prefix: String) extends ColumnWithNameAccessor {
  ...
  val grdtx: ColumnWithName = ColumnWithName(prefix + "grdtx")
...
object C_trugt extends C_trugt("") {
  def as(alias: String): C_trugt = new C_trugt(alias + ".")
}
