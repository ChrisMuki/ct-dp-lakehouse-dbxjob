...
object tcurx
    ...
    with ChangeKey[sr_raw.ct_gbl_p24.E_tcurx] {
  override def sourceTableSpec: TableSpec[sr_raw.ct_gbl_p24.E_tcurx] = sr_raw.ct_gbl_p24.tcurx
  override def sequenceBy: Column = col("_mk_created_at")

  override def preApplyMapping: Seq[(String, Column)] = Seq(
    ("currkey", col("currkey_string")),
    ("currdec", col("currdec_int"))
  )
}
...
sealed class C_tcurx(prefix: String) extends ColumnWithNameAccessor {
  val currkey: ColumnWithName = ColumnWithName(prefix + "currkey")
  val currdec: ColumnWithName = ColumnWithName(prefix + "currdec")
