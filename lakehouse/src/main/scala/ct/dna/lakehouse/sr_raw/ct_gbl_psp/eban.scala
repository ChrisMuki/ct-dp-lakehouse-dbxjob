// LOADED TABLE SPEC AUTO GENERATED:START
// Generated by LoadedTableSpecAstBuilder - DO NOT EDIT MANUALLY
package ct.dna.lakehouse.sr_raw.ct_gbl_psp

import ct.dna.lakehouse.core.model.ClusterBy
import ct.dna.lakehouse.core.model.Entity
import ct.dna.lakehouse.core.model.Entity._
import ct.dna.lakehouse.core.model.Loaded
import ct.dna.lakehouse.core.model.TableSpec

case class E_eban(
    @PK _mk_org: String,
    @PK _mk_site: String,
    @PK _mk_system: String,
    @PK _mk_instance: String,
    @PK _mk_partition: String,
    @PK _mk_file: String,
    @NotNull _mk_container: String,
    @NotNull _mk_account: String,
    @NotNull _mk_created_at: Timestamp,
    @PK _lh_id_in_message: Long,
    _lh_ingest_warning: String,
    mandt_string: String,
    banfn_string: String,
    bnfpo_string: String,
    bsart_string: String,
    bstyp_string: String,
    bsakz_string: String,
    loekz_string: String,
    statu_string: String,
    estkz_string: String,
    frgkz_string: String,
    frgzu_string: String,
    frgst_string: String,
    ekgrp_string: String,
    ernam_string: String,
    erdat_string: String,
    afnam_string: String,
    txz01_string: String,
    matnr_string: String,
    ematn_string: String,
    werks_string: String,
    lgort_string: String,
    bednr_string: String,
    matkl_string: String,
    reswk_string: String,
    @Decimal(13, 3) menge_decimal_13_3: BigDecimal,
    meins_string: String,
    @Decimal(13, 3) bumng_decimal_13_3: BigDecimal,
    badat_string: String,
    lpein_string: String,
    lfdat_string: String,
    frgdt_string: String,
    @Decimal(3, 0) webaz_decimal_3_0: BigDecimal,
    @Decimal(11, 2) preis_decimal_11_2: BigDecimal,
    @Decimal(5, 0) peinh_decimal_5_0: BigDecimal,
    pstyp_string: String,
    knttp_string: String,
    kzvbr_string: String,
    kflag_string: String,
    vrtkz_string: String,
    twrkz_string: String,
    wepos_string: String,
    weunb_string: String,
    repos_string: String,
    lifnr_string: String,
    flief_string: String,
    ekorg_string: String,
    vrtyp_string: String,
    konnr_string: String,
    ktpnr_string: String,
    infnr_string: String,
    zugba_string: String,
    qunum_string: String,
    qupos_string: String,
    dispo_string: String,
    sernr_string: String,
    bvdat_string: String,
    @Decimal(3, 0) batol_decimal_3_0: BigDecimal,
    @Decimal(7, 0) bvdrk_decimal_7_0: BigDecimal,
    ebeln_string: String,
    ebelp_string: String,
    bedat_string: String,
    @Decimal(13, 3) bsmng_decimal_13_3: BigDecimal,
    @Decimal(13, 2) limit_consumption_value_decimal_13_2: BigDecimal,
    lblni_string: String,
    bwtar_string: String,
    xoblr_string: String,
    ebakz_string: String,
    rsnum_string: String,
    sobkz_string: String,
    arsnr_string: String,
    arsps_string: String,
    fixkz_string: String,
    bmein_string: String,
    revlv_string: String,
    vorab_string: String,
    packno_string: String,
    kanba_string: String,
    bpueb_string: String,
    cuobj_string: String,
    frggr_string: String,
    frgrl_string: String,
    aktnr_string: String,
    charg_string: String,
    umsok_string: String,
    verid_string: String,
    fipos_string: String,
    fistl_string: String,
    geber_string: String,
    kzkfg_string: String,
    satnr_string: String,
    @Decimal(13, 3) mng02_decimal_13_3: BigDecimal,
    dat01_string: String,
    attyp_string: String,
    adrnr_string: String,
    adrn2_string: String,
    kunnr_string: String,
    emlif_string: String,
    lblkz_string: String,
    kzbws_string: String,
    waers_string: String,
    idnlf_string: String,
    gsfrg_string: String,
    mprof_string: String,
    kzfme_string: String,
    spras_string: String,
    techs_string: String,
    mfrpn_string: String,
    mfrnr_string: String,
    emnfr_string: String,
    fordn_string: String,
    fordp_string: String,
    @Decimal(3, 0) plifz_decimal_3_0: BigDecimal,
    berid_string: String,
    uzeit_string: String,
    fkber_string: String,
    grant_nbr_string: String,
    memory_string: String,
    banpr_string: String,
    @Decimal(15, 2) rlwrt_decimal_15_2: BigDecimal,
    blckd_string: String,
    revno_string: String,
    blckt_string: String,
    beswk_string: String,
    eprofile_string: String,
    eprefdoc_string: String,
    eprefitm_string: String,
    @Decimal(13, 3) gmmng_decimal_13_3: BigDecimal,
    wrtkz_string: String,
    reslo_string: String,
    kblnr_string: String,
    kblpos_string: String,
    prio_urg_string: String,
    prio_req_string: String,
    memorytype_string: String,
    anzsn_int: BoxedInt,
    @Decimal(4, 0) mhdrz_decimal_4_0: BigDecimal,
    iprkz_string: String,
    nodisp_string: String,
    srm_contract_id_string: String,
    srm_contract_itm_string: String,
    budget_pd_string: String,
    eldat_string: String,
    expert_mode_string: String,
    central_purreqn_proc_string: String,
    spe_crm_ref_so_string: String,
    spe_crm_ref_item_string: String,
    indelegateapproval_string: String,
    dummy_eban_incl_eew_ps_string: String,
    storenetworkid_string: String,
    storesupplierid_string: String,
    creationdate_string: String,
    creationtime_string: String,
    producttype_string: String,
    serviceperformer_string: String,
    iseopblocked_string: String,
    @Decimal(21, 7) ext_rev_tmstmp_decimal_21_7: BigDecimal,
    fmfgus_key_string: String,
    startdate_string: String,
    enddate_string: String,
    extmaterialforpurg_string: String,
    extfixedsupplierforpurg_string: String,
    extdesiredsupplierforpurg_string: String,
    extcontractforpurg_string: String,
    extcontractitemforpurg_string: String,
    extinforecordforpurg_string: String,
    extplantforpurg_string: String,
    extstoragelocationforpurg_string: String,
    extcompanycodeforpurg_string: String,
    extpurgorgforpurg_string: String,
    extsourcesystem_string: String,
    ext_be_source_sys_string: String,
    ext_be_prchangeindicator_string: String,
    extdoctypeforpurg_string: String,
    iscrreplicationbeforeapproval_string: String,
    mmpur_pr_cen_reqn_app_rpld_pr_string: String,
    extapprovalstatus_string: String,
    stagingfldextapprovalstatus_string: String,
    purchasereqnitemuniqueid_string: String,
    @Decimal(21, 7) lastchangedatetime_decimal_21_7: BigDecimal,
    isonbehalfcart_string: String,
    sdm_version_string: String,
    sakto_string: String,
    kostl_string: String,
    @Decimal(13, 2) expected_value_decimal_13_2: BigDecimal,
    @Decimal(13, 2) limit_amount_decimal_13_2: BigDecimal,
    contract_for_limit_string: String,
    contract_item_for_limit_string: String,
    deliveryaddrtype_string: String,
    mwskz_string: String,
    pfmtransdatafootprintuuid_binary: Array[Byte],
    hastradecomplianceissue_string: String,
    purreqndescription_string: String,
    isoutline_string: String,
    parent_item_no_string: String,
    outline_type_string: String,
    exlin_string: String,
    exsnr_string: String,
    advcode_string: String,
    stacode_string: String,
    banfn_cs_string: String,
    bnfpo_cs_string: String,
    item_cs_string: String,
    @Decimal(13, 3) bsmng_snd_decimal_13_3: BigDecimal,
    no_mard_data_string: String,
    admoi_string: String,
    adpri_string: String,
    lprio_string: String,
    adacn_string: String,
    addns_string: String,
    serru_string: String,
    disub_sobkz_string: String,
    disub_pspnr_string: String,
    disub_kunnr_string: String,
    disub_vbeln_string: String,
    disub_posnr_string: String,
    disub_owner_string: String,
    fldlogssupplyprocess_string: String,
    fldlogsdelivisheldonshore_string: String,
    fldlogsversion_string: String,
    fsh_season_year_string: String,
    fsh_season_string: String,
    fsh_collection_string: String,
    fsh_theme_string: String,
    fsh_vas_rel_string: String,
    fsh_vas_prnt_id_string: String,
    fsh_transaction_string: String,
    fsh_item_group_string: String,
    fsh_item_string: String,
    iuid_relevant_string: String,
    sc_sr_item_key_int: BoxedInt,
    sc_catalogid_string: String,
    sc_catalogitem_string: String,
    sc_requestor_string: String,
    sc_author_string: String,
    sgt_scat_string: String,
    sgt_rcat_string: String,
    wrf_charstc1_string: String,
    wrf_charstc2_string: String,
    wrf_charstc3_string: String
) extends Entity

object eban
    extends TableSpec[E_eban](
      enableChangeDataFeed = true,
      clusterBy = ClusterBy.Columns("_mk_instance", "_mk_partition", "_mk_file"),
      timetravelDays = 35
    )
    with Loaded
// LOADED TABLE SPEC AUTO GENERATED:END

// COLUMN ACCESSOR AUTO GENERATED:START
// Generated by ColumnWithNameAccessorEmbeddedAstBuilder - DO NOT EDIT MANUALLY
import ct.dna.lakehouse.core.model.ColumnWithName
import ct.dna.lakehouse.core.model.ColumnWithNameAccessor

sealed class C_eban(prefix: String) extends ColumnWithNameAccessor {
  val _mk_org: ColumnWithName = ColumnWithName(prefix + "_mk_org")
  val _mk_site: ColumnWithName = ColumnWithName(prefix + "_mk_site")
  val _mk_system: ColumnWithName = ColumnWithName(prefix + "_mk_system")
  val _mk_instance: ColumnWithName = ColumnWithName(prefix + "_mk_instance")
  val _mk_partition: ColumnWithName = ColumnWithName(prefix + "_mk_partition")
  val _mk_file: ColumnWithName = ColumnWithName(prefix + "_mk_file")
  val _mk_container: ColumnWithName = ColumnWithName(prefix + "_mk_container")
  val _mk_account: ColumnWithName = ColumnWithName(prefix + "_mk_account")
  val _mk_created_at: ColumnWithName = ColumnWithName(prefix + "_mk_created_at")
  val _lh_id_in_message: ColumnWithName = ColumnWithName(prefix + "_lh_id_in_message")
  val _lh_ingest_warning: ColumnWithName = ColumnWithName(prefix + "_lh_ingest_warning")
  val mandt_string: ColumnWithName = ColumnWithName(prefix + "mandt_string")
  val banfn_string: ColumnWithName = ColumnWithName(prefix + "banfn_string")
  val bnfpo_string: ColumnWithName = ColumnWithName(prefix + "bnfpo_string")
  val bsart_string: ColumnWithName = ColumnWithName(prefix + "bsart_string")
  val bstyp_string: ColumnWithName = ColumnWithName(prefix + "bstyp_string")
  val bsakz_string: ColumnWithName = ColumnWithName(prefix + "bsakz_string")
  val loekz_string: ColumnWithName = ColumnWithName(prefix + "loekz_string")
  val statu_string: ColumnWithName = ColumnWithName(prefix + "statu_string")
  val estkz_string: ColumnWithName = ColumnWithName(prefix + "estkz_string")
  val frgkz_string: ColumnWithName = ColumnWithName(prefix + "frgkz_string")
  val frgzu_string: ColumnWithName = ColumnWithName(prefix + "frgzu_string")
  val frgst_string: ColumnWithName = ColumnWithName(prefix + "frgst_string")
  val ekgrp_string: ColumnWithName = ColumnWithName(prefix + "ekgrp_string")
  val ernam_string: ColumnWithName = ColumnWithName(prefix + "ernam_string")
  val erdat_string: ColumnWithName = ColumnWithName(prefix + "erdat_string")
  val afnam_string: ColumnWithName = ColumnWithName(prefix + "afnam_string")
  val txz01_string: ColumnWithName = ColumnWithName(prefix + "txz01_string")
  val matnr_string: ColumnWithName = ColumnWithName(prefix + "matnr_string")
  val ematn_string: ColumnWithName = ColumnWithName(prefix + "ematn_string")
  val werks_string: ColumnWithName = ColumnWithName(prefix + "werks_string")
  val lgort_string: ColumnWithName = ColumnWithName(prefix + "lgort_string")
  val bednr_string: ColumnWithName = ColumnWithName(prefix + "bednr_string")
  val matkl_string: ColumnWithName = ColumnWithName(prefix + "matkl_string")
  val reswk_string: ColumnWithName = ColumnWithName(prefix + "reswk_string")
  val menge_decimal_13_3: ColumnWithName = ColumnWithName(prefix + "menge_decimal_13_3")
  val meins_string: ColumnWithName = ColumnWithName(prefix + "meins_string")
  val bumng_decimal_13_3: ColumnWithName = ColumnWithName(prefix + "bumng_decimal_13_3")
  val badat_string: ColumnWithName = ColumnWithName(prefix + "badat_string")
  val lpein_string: ColumnWithName = ColumnWithName(prefix + "lpein_string")
  val lfdat_string: ColumnWithName = ColumnWithName(prefix + "lfdat_string")
  val frgdt_string: ColumnWithName = ColumnWithName(prefix + "frgdt_string")
  val webaz_decimal_3_0: ColumnWithName = ColumnWithName(prefix + "webaz_decimal_3_0")
  val preis_decimal_11_2: ColumnWithName = ColumnWithName(prefix + "preis_decimal_11_2")
  val peinh_decimal_5_0: ColumnWithName = ColumnWithName(prefix + "peinh_decimal_5_0")
  val pstyp_string: ColumnWithName = ColumnWithName(prefix + "pstyp_string")
  val knttp_string: ColumnWithName = ColumnWithName(prefix + "knttp_string")
  val kzvbr_string: ColumnWithName = ColumnWithName(prefix + "kzvbr_string")
  val kflag_string: ColumnWithName = ColumnWithName(prefix + "kflag_string")
  val vrtkz_string: ColumnWithName = ColumnWithName(prefix + "vrtkz_string")
  val twrkz_string: ColumnWithName = ColumnWithName(prefix + "twrkz_string")
  val wepos_string: ColumnWithName = ColumnWithName(prefix + "wepos_string")
  val weunb_string: ColumnWithName = ColumnWithName(prefix + "weunb_string")
  val repos_string: ColumnWithName = ColumnWithName(prefix + "repos_string")
  val lifnr_string: ColumnWithName = ColumnWithName(prefix + "lifnr_string")
  val flief_string: ColumnWithName = ColumnWithName(prefix + "flief_string")
  val ekorg_string: ColumnWithName = ColumnWithName(prefix + "ekorg_string")
  val vrtyp_string: ColumnWithName = ColumnWithName(prefix + "vrtyp_string")
  val konnr_string: ColumnWithName = ColumnWithName(prefix + "konnr_string")
  val ktpnr_string: ColumnWithName = ColumnWithName(prefix + "ktpnr_string")
  val infnr_string: ColumnWithName = ColumnWithName(prefix + "infnr_string")
  val zugba_string: ColumnWithName = ColumnWithName(prefix + "zugba_string")
  val qunum_string: ColumnWithName = ColumnWithName(prefix + "qunum_string")
  val qupos_string: ColumnWithName = ColumnWithName(prefix + "qupos_string")
  val dispo_string: ColumnWithName = ColumnWithName(prefix + "dispo_string")
  val sernr_string: ColumnWithName = ColumnWithName(prefix + "sernr_string")
  val bvdat_string: ColumnWithName = ColumnWithName(prefix + "bvdat_string")
  val batol_decimal_3_0: ColumnWithName = ColumnWithName(prefix + "batol_decimal_3_0")
  val bvdrk_decimal_7_0: ColumnWithName = ColumnWithName(prefix + "bvdrk_decimal_7_0")
  val ebeln_string: ColumnWithName = ColumnWithName(prefix + "ebeln_string")
  val ebelp_string: ColumnWithName = ColumnWithName(prefix + "ebelp_string")
  val bedat_string: ColumnWithName = ColumnWithName(prefix + "bedat_string")
  val bsmng_decimal_13_3: ColumnWithName = ColumnWithName(prefix + "bsmng_decimal_13_3")
  val limit_consumption_value_decimal_13_2: ColumnWithName = ColumnWithName(prefix + "limit_consumption_value_decimal_13_2")
  val lblni_string: ColumnWithName = ColumnWithName(prefix + "lblni_string")
  val bwtar_string: ColumnWithName = ColumnWithName(prefix + "bwtar_string")
  val xoblr_string: ColumnWithName = ColumnWithName(prefix + "xoblr_string")
  val ebakz_string: ColumnWithName = ColumnWithName(prefix + "ebakz_string")
  val rsnum_string: ColumnWithName = ColumnWithName(prefix + "rsnum_string")
  val sobkz_string: ColumnWithName = ColumnWithName(prefix + "sobkz_string")
  val arsnr_string: ColumnWithName = ColumnWithName(prefix + "arsnr_string")
  val arsps_string: ColumnWithName = ColumnWithName(prefix + "arsps_string")
  val fixkz_string: ColumnWithName = ColumnWithName(prefix + "fixkz_string")
  val bmein_string: ColumnWithName = ColumnWithName(prefix + "bmein_string")
  val revlv_string: ColumnWithName = ColumnWithName(prefix + "revlv_string")
  val vorab_string: ColumnWithName = ColumnWithName(prefix + "vorab_string")
  val packno_string: ColumnWithName = ColumnWithName(prefix + "packno_string")
  val kanba_string: ColumnWithName = ColumnWithName(prefix + "kanba_string")
  val bpueb_string: ColumnWithName = ColumnWithName(prefix + "bpueb_string")
  val cuobj_string: ColumnWithName = ColumnWithName(prefix + "cuobj_string")
  val frggr_string: ColumnWithName = ColumnWithName(prefix + "frggr_string")
  val frgrl_string: ColumnWithName = ColumnWithName(prefix + "frgrl_string")
  val aktnr_string: ColumnWithName = ColumnWithName(prefix + "aktnr_string")
  val charg_string: ColumnWithName = ColumnWithName(prefix + "charg_string")
  val umsok_string: ColumnWithName = ColumnWithName(prefix + "umsok_string")
  val verid_string: ColumnWithName = ColumnWithName(prefix + "verid_string")
  val fipos_string: ColumnWithName = ColumnWithName(prefix + "fipos_string")
  val fistl_string: ColumnWithName = ColumnWithName(prefix + "fistl_string")
  val geber_string: ColumnWithName = ColumnWithName(prefix + "geber_string")
  val kzkfg_string: ColumnWithName = ColumnWithName(prefix + "kzkfg_string")
  val satnr_string: ColumnWithName = ColumnWithName(prefix + "satnr_string")
  val mng02_decimal_13_3: ColumnWithName = ColumnWithName(prefix + "mng02_decimal_13_3")
  val dat01_string: ColumnWithName = ColumnWithName(prefix + "dat01_string")
  val attyp_string: ColumnWithName = ColumnWithName(prefix + "attyp_string")
  val adrnr_string: ColumnWithName = ColumnWithName(prefix + "adrnr_string")
  val adrn2_string: ColumnWithName = ColumnWithName(prefix + "adrn2_string")
  val kunnr_string: ColumnWithName = ColumnWithName(prefix + "kunnr_string")
  val emlif_string: ColumnWithName = ColumnWithName(prefix + "emlif_string")
  val lblkz_string: ColumnWithName = ColumnWithName(prefix + "lblkz_string")
  val kzbws_string: ColumnWithName = ColumnWithName(prefix + "kzbws_string")
  val waers_string: ColumnWithName = ColumnWithName(prefix + "waers_string")
  val idnlf_string: ColumnWithName = ColumnWithName(prefix + "idnlf_string")
  val gsfrg_string: ColumnWithName = ColumnWithName(prefix + "gsfrg_string")
  val mprof_string: ColumnWithName = ColumnWithName(prefix + "mprof_string")
  val kzfme_string: ColumnWithName = ColumnWithName(prefix + "kzfme_string")
  val spras_string: ColumnWithName = ColumnWithName(prefix + "spras_string")
  val techs_string: ColumnWithName = ColumnWithName(prefix + "techs_string")
  val mfrpn_string: ColumnWithName = ColumnWithName(prefix + "mfrpn_string")
  val mfrnr_string: ColumnWithName = ColumnWithName(prefix + "mfrnr_string")
  val emnfr_string: ColumnWithName = ColumnWithName(prefix + "emnfr_string")
  val fordn_string: ColumnWithName = ColumnWithName(prefix + "fordn_string")
  val fordp_string: ColumnWithName = ColumnWithName(prefix + "fordp_string")
  val plifz_decimal_3_0: ColumnWithName = ColumnWithName(prefix + "plifz_decimal_3_0")
  val berid_string: ColumnWithName = ColumnWithName(prefix + "berid_string")
  val uzeit_string: ColumnWithName = ColumnWithName(prefix + "uzeit_string")
  val fkber_string: ColumnWithName = ColumnWithName(prefix + "fkber_string")
  val grant_nbr_string: ColumnWithName = ColumnWithName(prefix + "grant_nbr_string")
  val memory_string: ColumnWithName = ColumnWithName(prefix + "memory_string")
  val banpr_string: ColumnWithName = ColumnWithName(prefix + "banpr_string")
  val rlwrt_decimal_15_2: ColumnWithName = ColumnWithName(prefix + "rlwrt_decimal_15_2")
  val blckd_string: ColumnWithName = ColumnWithName(prefix + "blckd_string")
  val revno_string: ColumnWithName = ColumnWithName(prefix + "revno_string")
  val blckt_string: ColumnWithName = ColumnWithName(prefix + "blckt_string")
  val beswk_string: ColumnWithName = ColumnWithName(prefix + "beswk_string")
  val eprofile_string: ColumnWithName = ColumnWithName(prefix + "eprofile_string")
  val eprefdoc_string: ColumnWithName = ColumnWithName(prefix + "eprefdoc_string")
  val eprefitm_string: ColumnWithName = ColumnWithName(prefix + "eprefitm_string")
  val gmmng_decimal_13_3: ColumnWithName = ColumnWithName(prefix + "gmmng_decimal_13_3")
  val wrtkz_string: ColumnWithName = ColumnWithName(prefix + "wrtkz_string")
  val reslo_string: ColumnWithName = ColumnWithName(prefix + "reslo_string")
  val kblnr_string: ColumnWithName = ColumnWithName(prefix + "kblnr_string")
  val kblpos_string: ColumnWithName = ColumnWithName(prefix + "kblpos_string")
  val prio_urg_string: ColumnWithName = ColumnWithName(prefix + "prio_urg_string")
  val prio_req_string: ColumnWithName = ColumnWithName(prefix + "prio_req_string")
  val memorytype_string: ColumnWithName = ColumnWithName(prefix + "memorytype_string")
  val anzsn_int: ColumnWithName = ColumnWithName(prefix + "anzsn_int")
  val mhdrz_decimal_4_0: ColumnWithName = ColumnWithName(prefix + "mhdrz_decimal_4_0")
  val iprkz_string: ColumnWithName = ColumnWithName(prefix + "iprkz_string")
  val nodisp_string: ColumnWithName = ColumnWithName(prefix + "nodisp_string")
  val srm_contract_id_string: ColumnWithName = ColumnWithName(prefix + "srm_contract_id_string")
  val srm_contract_itm_string: ColumnWithName = ColumnWithName(prefix + "srm_contract_itm_string")
  val budget_pd_string: ColumnWithName = ColumnWithName(prefix + "budget_pd_string")
  val eldat_string: ColumnWithName = ColumnWithName(prefix + "eldat_string")
  val expert_mode_string: ColumnWithName = ColumnWithName(prefix + "expert_mode_string")
  val central_purreqn_proc_string: ColumnWithName = ColumnWithName(prefix + "central_purreqn_proc_string")
  val spe_crm_ref_so_string: ColumnWithName = ColumnWithName(prefix + "spe_crm_ref_so_string")
  val spe_crm_ref_item_string: ColumnWithName = ColumnWithName(prefix + "spe_crm_ref_item_string")
  val indelegateapproval_string: ColumnWithName = ColumnWithName(prefix + "indelegateapproval_string")
  val dummy_eban_incl_eew_ps_string: ColumnWithName = ColumnWithName(prefix + "dummy_eban_incl_eew_ps_string")
  val storenetworkid_string: ColumnWithName = ColumnWithName(prefix + "storenetworkid_string")
  val storesupplierid_string: ColumnWithName = ColumnWithName(prefix + "storesupplierid_string")
  val creationdate_string: ColumnWithName = ColumnWithName(prefix + "creationdate_string")
  val creationtime_string: ColumnWithName = ColumnWithName(prefix + "creationtime_string")
  val producttype_string: ColumnWithName = ColumnWithName(prefix + "producttype_string")
  val serviceperformer_string: ColumnWithName = ColumnWithName(prefix + "serviceperformer_string")
  val iseopblocked_string: ColumnWithName = ColumnWithName(prefix + "iseopblocked_string")
  val ext_rev_tmstmp_decimal_21_7: ColumnWithName = ColumnWithName(prefix + "ext_rev_tmstmp_decimal_21_7")
  val fmfgus_key_string: ColumnWithName = ColumnWithName(prefix + "fmfgus_key_string")
  val startdate_string: ColumnWithName = ColumnWithName(prefix + "startdate_string")
  val enddate_string: ColumnWithName = ColumnWithName(prefix + "enddate_string")
  val extmaterialforpurg_string: ColumnWithName = ColumnWithName(prefix + "extmaterialforpurg_string")
  val extfixedsupplierforpurg_string: ColumnWithName = ColumnWithName(prefix + "extfixedsupplierforpurg_string")
  val extdesiredsupplierforpurg_string: ColumnWithName = ColumnWithName(prefix + "extdesiredsupplierforpurg_string")
  val extcontractforpurg_string: ColumnWithName = ColumnWithName(prefix + "extcontractforpurg_string")
  val extcontractitemforpurg_string: ColumnWithName = ColumnWithName(prefix + "extcontractitemforpurg_string")
  val extinforecordforpurg_string: ColumnWithName = ColumnWithName(prefix + "extinforecordforpurg_string")
  val extplantforpurg_string: ColumnWithName = ColumnWithName(prefix + "extplantforpurg_string")
  val extstoragelocationforpurg_string: ColumnWithName = ColumnWithName(prefix + "extstoragelocationforpurg_string")
  val extcompanycodeforpurg_string: ColumnWithName = ColumnWithName(prefix + "extcompanycodeforpurg_string")
  val extpurgorgforpurg_string: ColumnWithName = ColumnWithName(prefix + "extpurgorgforpurg_string")
  val extsourcesystem_string: ColumnWithName = ColumnWithName(prefix + "extsourcesystem_string")
  val ext_be_source_sys_string: ColumnWithName = ColumnWithName(prefix + "ext_be_source_sys_string")
  val ext_be_prchangeindicator_string: ColumnWithName = ColumnWithName(prefix + "ext_be_prchangeindicator_string")
  val extdoctypeforpurg_string: ColumnWithName = ColumnWithName(prefix + "extdoctypeforpurg_string")
  val iscrreplicationbeforeapproval_string: ColumnWithName = ColumnWithName(prefix + "iscrreplicationbeforeapproval_string")
  val mmpur_pr_cen_reqn_app_rpld_pr_string: ColumnWithName = ColumnWithName(prefix + "mmpur_pr_cen_reqn_app_rpld_pr_string")
  val extapprovalstatus_string: ColumnWithName = ColumnWithName(prefix + "extapprovalstatus_string")
  val stagingfldextapprovalstatus_string: ColumnWithName = ColumnWithName(prefix + "stagingfldextapprovalstatus_string")
  val purchasereqnitemuniqueid_string: ColumnWithName = ColumnWithName(prefix + "purchasereqnitemuniqueid_string")
  val lastchangedatetime_decimal_21_7: ColumnWithName = ColumnWithName(prefix + "lastchangedatetime_decimal_21_7")
  val isonbehalfcart_string: ColumnWithName = ColumnWithName(prefix + "isonbehalfcart_string")
  val sdm_version_string: ColumnWithName = ColumnWithName(prefix + "sdm_version_string")
  val sakto_string: ColumnWithName = ColumnWithName(prefix + "sakto_string")
  val kostl_string: ColumnWithName = ColumnWithName(prefix + "kostl_string")
  val expected_value_decimal_13_2: ColumnWithName = ColumnWithName(prefix + "expected_value_decimal_13_2")
  val limit_amount_decimal_13_2: ColumnWithName = ColumnWithName(prefix + "limit_amount_decimal_13_2")
  val contract_for_limit_string: ColumnWithName = ColumnWithName(prefix + "contract_for_limit_string")
  val contract_item_for_limit_string: ColumnWithName = ColumnWithName(prefix + "contract_item_for_limit_string")
  val deliveryaddrtype_string: ColumnWithName = ColumnWithName(prefix + "deliveryaddrtype_string")
  val mwskz_string: ColumnWithName = ColumnWithName(prefix + "mwskz_string")
  val pfmtransdatafootprintuuid_binary: ColumnWithName = ColumnWithName(prefix + "pfmtransdatafootprintuuid_binary")
  val hastradecomplianceissue_string: ColumnWithName = ColumnWithName(prefix + "hastradecomplianceissue_string")
  val purreqndescription_string: ColumnWithName = ColumnWithName(prefix + "purreqndescription_string")
  val isoutline_string: ColumnWithName = ColumnWithName(prefix + "isoutline_string")
  val parent_item_no_string: ColumnWithName = ColumnWithName(prefix + "parent_item_no_string")
  val outline_type_string: ColumnWithName = ColumnWithName(prefix + "outline_type_string")
  val exlin_string: ColumnWithName = ColumnWithName(prefix + "exlin_string")
  val exsnr_string: ColumnWithName = ColumnWithName(prefix + "exsnr_string")
  val advcode_string: ColumnWithName = ColumnWithName(prefix + "advcode_string")
  val stacode_string: ColumnWithName = ColumnWithName(prefix + "stacode_string")
  val banfn_cs_string: ColumnWithName = ColumnWithName(prefix + "banfn_cs_string")
  val bnfpo_cs_string: ColumnWithName = ColumnWithName(prefix + "bnfpo_cs_string")
  val item_cs_string: ColumnWithName = ColumnWithName(prefix + "item_cs_string")
  val bsmng_snd_decimal_13_3: ColumnWithName = ColumnWithName(prefix + "bsmng_snd_decimal_13_3")
  val no_mard_data_string: ColumnWithName = ColumnWithName(prefix + "no_mard_data_string")
  val admoi_string: ColumnWithName = ColumnWithName(prefix + "admoi_string")
  val adpri_string: ColumnWithName = ColumnWithName(prefix + "adpri_string")
  val lprio_string: ColumnWithName = ColumnWithName(prefix + "lprio_string")
  val adacn_string: ColumnWithName = ColumnWithName(prefix + "adacn_string")
  val addns_string: ColumnWithName = ColumnWithName(prefix + "addns_string")
  val serru_string: ColumnWithName = ColumnWithName(prefix + "serru_string")
  val disub_sobkz_string: ColumnWithName = ColumnWithName(prefix + "disub_sobkz_string")
  val disub_pspnr_string: ColumnWithName = ColumnWithName(prefix + "disub_pspnr_string")
  val disub_kunnr_string: ColumnWithName = ColumnWithName(prefix + "disub_kunnr_string")
  val disub_vbeln_string: ColumnWithName = ColumnWithName(prefix + "disub_vbeln_string")
  val disub_posnr_string: ColumnWithName = ColumnWithName(prefix + "disub_posnr_string")
  val disub_owner_string: ColumnWithName = ColumnWithName(prefix + "disub_owner_string")
  val fldlogssupplyprocess_string: ColumnWithName = ColumnWithName(prefix + "fldlogssupplyprocess_string")
  val fldlogsdelivisheldonshore_string: ColumnWithName = ColumnWithName(prefix + "fldlogsdelivisheldonshore_string")
  val fldlogsversion_string: ColumnWithName = ColumnWithName(prefix + "fldlogsversion_string")
  val fsh_season_year_string: ColumnWithName = ColumnWithName(prefix + "fsh_season_year_string")
  val fsh_season_string: ColumnWithName = ColumnWithName(prefix + "fsh_season_string")
  val fsh_collection_string: ColumnWithName = ColumnWithName(prefix + "fsh_collection_string")
  val fsh_theme_string: ColumnWithName = ColumnWithName(prefix + "fsh_theme_string")
  val fsh_vas_rel_string: ColumnWithName = ColumnWithName(prefix + "fsh_vas_rel_string")
  val fsh_vas_prnt_id_string: ColumnWithName = ColumnWithName(prefix + "fsh_vas_prnt_id_string")
  val fsh_transaction_string: ColumnWithName = ColumnWithName(prefix + "fsh_transaction_string")
  val fsh_item_group_string: ColumnWithName = ColumnWithName(prefix + "fsh_item_group_string")
  val fsh_item_string: ColumnWithName = ColumnWithName(prefix + "fsh_item_string")
  val iuid_relevant_string: ColumnWithName = ColumnWithName(prefix + "iuid_relevant_string")
  val sc_sr_item_key_int: ColumnWithName = ColumnWithName(prefix + "sc_sr_item_key_int")
  val sc_catalogid_string: ColumnWithName = ColumnWithName(prefix + "sc_catalogid_string")
  val sc_catalogitem_string: ColumnWithName = ColumnWithName(prefix + "sc_catalogitem_string")
  val sc_requestor_string: ColumnWithName = ColumnWithName(prefix + "sc_requestor_string")
  val sc_author_string: ColumnWithName = ColumnWithName(prefix + "sc_author_string")
  val sgt_scat_string: ColumnWithName = ColumnWithName(prefix + "sgt_scat_string")
  val sgt_rcat_string: ColumnWithName = ColumnWithName(prefix + "sgt_rcat_string")
  val wrf_charstc1_string: ColumnWithName = ColumnWithName(prefix + "wrf_charstc1_string")
  val wrf_charstc2_string: ColumnWithName = ColumnWithName(prefix + "wrf_charstc2_string")
  val wrf_charstc3_string: ColumnWithName = ColumnWithName(prefix + "wrf_charstc3_string")
}

object C_eban extends C_eban("") {
  def as(alias: String): C_eban = new C_eban(alias + ".")
}
// COLUMN ACCESSOR AUTO GENERATED:END
