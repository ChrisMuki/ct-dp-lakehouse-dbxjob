...
object t023t extends TableSpec[DmT023T] with Updated.ByOneTransaction {
  ...
  override def executeTransaction(
    ...
    // When the concatenated wgbez result is blank (both languages absent), fall back to "No Entry".
    val newWgbezValue = when(concat_ws("~~", newD, newE) === "", lit("No Entry")).otherwise(concat_ws("~~", newD, newE))
    ...
    val insertWgbezValue =
      when(concat_ws("~~", source._value_d, source._value_e) === "", lit("No Entry")).otherwise(concat_ws("~~", source._value_d, source._value_e))
