...
object marc extends TableSpec[DmMarc] with Updated.ByOneTransaction {
  ...
  /** Project a per-feed `lastOfKey()` slice down to the columns the merge actually consumes:
    ...
    *   - cleansed `stawn` / `steuc` (dots and whitespace stripped); `stawn` falls back to `"No Entry"` when null or blank after cleansing
  ...
  private def projectChanges(lastOfKey: org.apache.spark.sql.DataFrame): org.apache.spark.sql.DataFrame =
    lastOfKey
      ...
      .select(
        ...
        when(regexp_replace(col("stawn"), "\\.|\\s", "").isNull || regexp_replace(col("stawn"), "\\.|\\s", "") === "", lit("No Entry"))
          .otherwise(regexp_replace(col("stawn"), "\\.|\\s", ""))
          .as("stawn"),
