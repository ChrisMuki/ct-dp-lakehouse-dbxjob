...
object mara extends TableSpec[DmMara] with Updated.ByOneTransaction {
  ...
  /** Project a per-feed `lastOfKey()` slice down to the columns the merge actually consumes:
    ...
    *   - all `DmMara` value columns, with null/empty → "No Entry" coalescing applied to `mtart`, `matkl`, `wrkst`
  ...
  private def projectChanges(lastOfKey: org.apache.spark.sql.DataFrame): org.apache.spark.sql.DataFrame =
    lastOfKey
      ...
      .select(
        ...
        col("_mk_instance"),
        col("matnr"),
        when(col("mtart").isNull || col("mtart") === "", lit("No Entry")).otherwise(col("mtart")).as("mtart"),
        when(col("matkl").isNull || col("matkl") === "", lit("No Entry")).otherwise(col("matkl")).as("matkl"),
        col("ersda"),
        col("pstat"),
        col("vpsta"),
        col("lvorm"),
        ...
        col("formt"),
        col("groes"),
        when(col("wrkst").isNull || col("wrkst") === "", lit("No Entry")).otherwise(col("wrkst")).as("wrkst"),
        col("normt"),
        col("brgew"),
        col("ntgew"),
        col("gewei"),
    ...
    val snapshotSystems: Set[String] = feeds
      .collect {
        case (_, feed) if feed.isSnapshot =>
          ...
          feed.snapshot(Seq("_mk_system")).select(C_mara._mk_system).limit(1).collect().map(_.getString(0)).toSet
      }
      .flatten
      .toSet
